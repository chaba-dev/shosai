...
- the covered / manifest-satisfied / pending acceptance rows with owners, and a
  ...
  (package 1C: `FM-21`, whose Hebrew and Arabic runs are blank under the pinned
  capture font set). The four rows that carried a reason before this round
  (`FM-01`, `FM-02`, `FM-13`, `RD-06`) are now covered by the rich-content
  fixtures, the manual-zoom captures, the CBZ fit-width capture and the
  injected-pointer hover captures;
...
The shared generated tree carries the multi-chapter English and Japanese EPUBs,
a four-page and a two-page artwork-only PDF and a twelve-page and a three-page
CBZ that the reader captures open, and the reused conformance EPUB stays in the
tree for the other packages. Which fixture each capture uses is recorded per
capture in the manifest.

Package 1C adds one generated fixture of its own: `library/featured/reader-marks.epub`,
one short chapter that carries the marks `FM-01` names (a heading, body text, a
list, a quote and a link) and a 64×64 document image filled with a single colour
(`READER_MARKS_COLOUR`) that no chrome token uses, so `FM-18`'s reference can
require that exact colour in the rendered page. It is generated by the same
deterministic generator as the rest of the tree, and it deliberately carries no
italic on the interface family, which is why it renders where the repository's
sampler book cannot.

The rich-content captures open from package 1C's own base seed: the shared
library plus six reused conformance fixtures (`bidi`, `fonts`, `fonts-isolation`,
`links`, `mathml`, `table`) and the generated marks fixture. It is a separate
store, so the shared seed's order — and therefore package 1B's evidence — is
untouched. `conformance.epub` and `css-cascade.epub` are deliberately not opened
by a rich capture: they cannot render under the pinned capture font environment
(see the limitations). `conformance.epub` is still part of the shared seed the
rich base inherits, and its reuse record is package 1B's; the 1C reused-fixture
list records the reader-specific additions. `nested-image.epub` is not opened
either: its only images are 1 px square, so a capture of it would record a
degenerate 1 px layout rather than a document image.
...
`FM-17`, `FM-19` and `FM-20`. The rows whose authority is not Iced at all are
also recorded in `non_iced_authority`, with the authority that owns them.

`FM-18` (EPUB pages keep document colours through compositing) is covered by
`rd-epub-marks-w1280-en`, an `1C-EPUB-PAG` capture whose generated fixture embeds
an image in a colour the chrome never uses and whose regression test requires
that exact colour in the rendered page, so a tinted or monochrome composite fails
instead of passing unnoticed. The specification names `F4` (`conformance.epub`)
and `F7` (`mathml.epub`) for the row, but `F4` cannot render under the pinned
capture font environment and `F7` carries no document colour, so the evidence
comes from the generated fixture instead of a fabricated `F4` render. Acceptance
stays with 5G: this is the row's Iced reference, not its acceptance evidence. The
row's *document text* colour half cannot be evidenced here at all — the shared
style model has no colour field, so document text is painted with the reader
palette (see the limitations).

`FM-21` is the one row whose reference coverage is deliberately partial, and it
carries that as a `reason` in the matrix: `rd-epub-bidi-w1280-mix` renders the
reused `F5` fixture at the specification's `MIX` configuration, so its
right-to-left paragraphs keep their direction and its Latin and Japanese runs
compose. Two parts of the row are not visible there: its Hebrew and Arabic runs
...
list composition is left-to-right (`app/epub_view.rs`). Evidencing those halves
needs a Hebrew/Arabic face in the capture font set and an RTL-aware list
composition — owner decisions about the capture contract and renderer work, not
something this package may add on its own.

Four of the rows that used to carry a `reason` are closed by captures, not by
weakening the row, and one row (`FM-21`) carries a new partial reason:

- `FM-01`: the generated marks fixture carries the row's styled text, headings,
  list, quote, styled link and page number, and `rd-epub-links-w1280-en` adds the
  reused conformance link fixture's anchor text and its cross-chapter target
  (its block-level anchors are not link-styled: see the limitations);
- `FM-02` and `FM-13`: fit-page, fit-width and manual raster zoom are all
  referenced. `rd-pdf-fit-width-w1280-en` and `rd-cbz-fit-width-w1280-en` are the
  fit-width halves, and `rd-pdf-manual-zoom-w1280-en` and
  `rd-cbz-manual-zoom-w1280-en` reach manual zoom the way the application does —
  one `Message::ZoomIn` from the fit-page baseline — with the recorded scale
  re-derived from the fixture's own page size by a regression test;
- `RD-06`: `rd-chrome-hover-edge-w1280-en` and `rd-chrome-hover-edge-c390-ja`
  draw the frame with the pointer delivered at the enabled next-page edge
  control — the wide 36 px and the compact 28 px configuration — so each capture
  shows the disabled control, the enabled control and the hovered control in one
  state. A regression test requires each image to differ from the same state
  without a pointer, to differ only inside the edge-control strip, and to paint
  the control's own hovered background.

`FM-21`'s reason is recorded per row in the matrix as well:

- `FM-21`: the `F5` bidi fixture's right-to-left paragraphs and its Latin and
  Japanese runs are referenced; its Hebrew and Arabic runs are blank under the
  pinned capture font set and its authored RTL list direction is not preserved
  by the left-to-right Iced list composition, so the row's shaping and RTL-list
  halves are not evidenced here and its acceptance stays with 5C.

The rows that remain partial in a *different* sense — a document text colour, a
paragraph-inline image or a block-level anchor's link styling that the reader
does not compose — are recorded in the known limitations rather than as a row
`reason`, because no capture could show them without new renderer functionality.
...
- the offscreen renderer has no scroll interaction and the application exposes no
  ...
  production page-navigation messages. The zoom captures
  (`rd-pdf-fit-width-w1280-en`, `rd-cbz-fit-width-w1280-en`,
  `rd-pdf-manual-zoom-w1280-en`, `rd-cbz-manual-zoom-w1280-en`) are the deliberate
  exceptions: their page is scaled past the viewport height, so they show the
  overflow and the vertical scrollbar instead of a page that fits. Tiled
  continuous seams (`FM-14`) stay with the Flutter/renderer packages;
...
- **two of the repository's conformance fixtures cannot render under the pinned
  capture font environment.** `conformance.epub` and `css-cascade.epub` both style
  `font-style: italic` on the default family, which has no italic face in the
  pinned set, and building the view panics inside the text stack
  (`no default font found`). This is production behavior, not a harness artifact:
  with only the application fonts registered, the fallback iterator runs out.
  The other twelve conformance fixtures render, and the rich captures use those;
  the two that cannot are recorded in `reader::limitations` and the fallback gap
  itself belongs with the renderer owner rather than being papered over here;
- **a block-level anchor loses its own `href`.** The parser's inline collector
  applies a link only to the spans *inside* an `a` element, so an `a` that is a
  direct child of a block container is composed as plain text. `links.epub`'s
  chapter is written that way, which is why `rd-epub-links-w1280-en` shows its
  anchor text without the link colour or underline while the paragraph link in
  `rd-epub-marks-w1280-en` renders normally; the gap belongs with the renderer
  owner;
- **the shared EPUB style model carries no colour field**, so document text is
  painted with the reader palette: no capture can show a CSS text colour, and
  `FM-18`'s Iced reference is the document *image* path instead (a document text
  colour is renderer work that belongs with 5G);
- **document images compose only where the parser keeps an image node.** A
  block-level `img` and an image inside a table cell render; an `img` inside a
  paragraph is dropped by the inline collector, which keeps text only, and a
  missing resource never reaches the page. `nested-image.epub`'s paragraph-inline
  image and its missing-image fallback are therefore not composed; the gap is
  recorded rather than hidden by a fabricated state;
- **the pinned capture font set covers Latin, Japanese and the math glyphs only**
  (Inter, NotoSansJP and the math font, with a fontconfig that names an empty
  directory). `FM-21`'s Hebrew and Arabic runs in `bidi.epub`, and the emoji in
  its mixed line, therefore render blank in `rd-epub-bidi-w1280-mix`; the capture
  shows the fixture's right-to-left paragraphs and its Latin/Japanese runs, not
  its Hebrew or Arabic shaping. The fixture's authored RTL list direction is not
  preserved either: the Iced list composition is left-to-right
  (`app/epub_view.rs`). Evidencing those halves needs a Hebrew/Arabic face in the
  capture font set and an RTL-aware list composition, which are changes to the
  capture contract and renderer work, and owner decisions.
